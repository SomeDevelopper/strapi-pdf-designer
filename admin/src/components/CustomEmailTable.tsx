...
const CustomEmailTable = ({
  ...
  const [duplicateConfirmationModal, setDuplicateConfirmationModal] =
    useState(false);
  ...
  const emailTemplatesHeaders = [
    {
      name: formatMessage({ id: getTranslation("table.header.name") }),
      value: "name",
    },
    {
      name: formatMessage({
        id: getTranslation("table.header.templateReferenceId"),
      }),
      value: "templateReferenceId",
    },
    {
      name: formatMessage({ id: getTranslation("table.header.createdAt") }),
      value: "createdAt",
    },
  ];
  if (data.length === 0) {
      ...
      <Box padding={8}>
        <EmptyStateLayout
          ...
          content={formatMessage({
            id: getTranslation("customTable.noDesigns"),
          })}
          action={
            <Button
              variant="secondary"
        ...
        <ImportExportActions
          data={data}
          reload={reload}
          handleTemplatesExport={handleTemplatesExport}
        />
      </Box>
    );
  }
  return (
    <>
      ...
      >
        <Dialog.Content>
          <Dialog.Header>
            {formatMessage({ id: getTranslation("confirm.title") })}
          </Dialog.Header>
          ...
          <Dialog.Footer>
            <Dialog.Cancel>
              <Button
                onClick={() => setDuplicateId(undefined)}
                fullWidth
                variant="tertiary"
              >
                Cancel
            ...
            <Dialog.Action>
              <Button
                onClick={() => handleTemplateDuplication()}
                fullWidth
      ...
      <Dialog.Root
        ...
        onOpenChange={() => setShowDeleteModal((s: boolean) => !s)}
      >
        <Dialog.Content>
          <Dialog.Header>
            {formatMessage({ id: getTranslation("confirm.title") })}
              ...
              <Tr key={idx}>
                ...
                <Td>
                  <Flex gap="10px" justifyContent="end">
                    <IconButton
                      label={formatMessage({
                        id: getTranslation("tooltip.edit"),
                      })}
                      onClick={() =>
                        navigate({ pathname: getUrl(`design/${entry.id}`) })
                      }
                    >
                      <Pencil />
                    </IconButton>
                    ...
                    <IconButton
                      label={formatMessage({
                        id: getTranslation("tooltip.downloadDesign"),
                      })}
                    ...
                    <IconButton
                      label={formatMessage({
                        id: getTranslation("tooltip.duplicate"),
                      })}
                      onClick={() => {
                    ...
                    <IconButton
                      ...
                      onClick={() => {
                        navigator.clipboard
                          .writeText(`${entry.templateReferenceId}`)
                          .then(
                            () => {
                              toggleNotification({
                                type: "success",
                                ...
                                message: formatMessage({
                                  id: getTranslation("success.copyTemplateId"),
                                }),
                              });
                            },
                            (err) => {
                    ...
                    <Box paddingLeft={1}>
                      <IconButton
                        label={formatMessage({
                          id: getTranslation("tooltip.delete"),
                        })}
                        onClick={() => {

...
const Designer = ({ isCore = false }: { isCore?: boolean }) => {
  ...
  const [isLoadingTestDesign, setIsLoadingTestDesign] = useState(false);
  const saveDesign = async () => {
    ...
    try {
      ...
      if (templateId === "new")
        navigate(`/plugins/${PLUGIN_ID}/design/${response?.id}`);
    } catch (err: any) {
      console.error(err);
  ...
  const downloadTestDesign = async () => {
    if (!templateId) {
      toggleNotification({
        type: "danger",
        title: translate("error"),
        message: "Error: Template ID is required to download the design.",
      });
      return;
    }
    try {
      setIsLoadingTestDesign(true);
      await downloadTestPdfDesign(templateId);
    } catch (e) {
      console.error("Error downloading test design:", e);
      toggleNotification({
        type: "danger",
        title: translate("error"),
        message: "Error",
  ...
  const init = async () => {
    if (
      ...
      (coreEmailType &&
        !["user-address-confirmation", "reset-password"].includes(
          coreEmailType
        )) ||
      templateId === "new"
    )
    ...
    if (coreEmailType && isEmpty(_templateData.design)) {
      ...
      _templateData.design = JSON.parse(
        JSON.stringify(standardEmailRegistrationTemplate).replace(
          "__PLACEHOLDER__",
          _message
        )
      );

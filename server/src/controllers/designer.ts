...
const controller = ({ strapi }: { strapi: Core.Strapi }) => ({
  ...
  getTemplatePdf: async (ctx) => {
    ...
    try {
      const buffer = await htmlToPdf(template.bodyHtml);
      ...
      strapi.log.error("Error generating PDF:", err);
      return ctx.internalServerError("Failed to generate PDF");
    }

    // transform function with callback to a promise
  },
    ...
    try {
      ...
      if (templateId === "new") {
        ...
        if (
          ...
          await strapi
            .documents(
              `plugin::${configImport.pluginName}.pdf-designer-template-version`
            )
            .create({
              data: {
      ...
      strapi.log.error("Error saving template:", error);
